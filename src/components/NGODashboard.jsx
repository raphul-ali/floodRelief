            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredRequests.map((req) => {
                ...
                return (
                  <div 
                    key={req.id} 
                    className={`rounded-2xl border p-4 sm:p-5 transition-all flex flex-col justify-between overflow-hidden shadow-md hover:shadow-lg ${cardBgStyle}`}
                  >
                    <div>
                      ...
                      <div className="flex items-center justify-between gap-2 pb-3 mb-3 border-b border-slate-200/80">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {req.verified ? (
                            <span className="px-3 py-1 rounded-full text-xs font-black bg-[#16a34a] text-white flex items-center gap-1 shadow-sm">
                              <ShieldCheck className="w-3.5 h-3.5" /> Verified
                            </span>
                          ) : isUrgent ? (
                            <span className="px-2.5 py-1 rounded-full text-xs font-black bg-red-600 text-white flex items-center gap-1 shadow-sm">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="text-[10px] font-mono text-slate-600 bg-slate-100 px-2 py-1 rounded-lg border border-slate-300 shrink-0">
                            {req.id}
                          </span>
                        </div>
                        ...
                        <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1.5">
                          <div className="flex items-start gap-2 text-xs text-slate-800 font-medium">
                            ...
                            <div>
                              <span className="font-black text-slate-900">{req.district}: </span>
                              <span className="font-semibold text-slate-700">{req.locationName || req.villageName}</span>
                              {req.pinCode && (
                                <span className="ml-1 px-1.5 py-0.2 bg-slate-200 text-slate-800 rounded font-mono text-[10px] border border-slate-300">
                                  PIN: {req.pinCode}
                                </span>
                              )}
                            </div>
                          ...
                          {req.latitude && req.longitude && (
                            <a
                              ...
                              className="mt-1.5 w-full py-1.5 px-3 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg text-xs font-extrabold flex items-center justify-center gap-1.5 transition-all shadow-2xs"
                            >
                        ...
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Supplies / Help Needed:</span>
